...
export async function POST(request: NextRequest) {
  ...
  try {
    const client = await lupaFetch<Record<string, unknown>>("/v1/clients/create", { method: "POST", body: JSON.stringify(withLupaScope({ firstName, lastName, email, phone, marketingCommsPreferences: [] }, "client")) });
    ...
    await lupaFetch<string>("/v1/request-booking", { method: "POST", body: JSON.stringify(withLupaScope({ petId, visitTypeId, start, end, notes: clean(input.notes, 1000) || undefined }, "booking")) });
